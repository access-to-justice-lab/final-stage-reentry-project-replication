...
forvalues filenum = 1/`imputations' {
	...
	***********************************
	import delimited using "./job_satData`filenum'.csv", clear  // 


	*********************
	*** Main IV Model ***
	*********************
	ivreg2 job_sat (i.expunge i.expunge#i.seq = i.treatment i.treatment#i.seq) ///
		white female age_enroll income full_time_lag unemploy_lag ps_employed local_unemploy i.seq i.year, ///
		cluster(stid)
	...
	* Time-varying LATEs
	...
	export delimited effect b se pvalue ll ul using "./jobSat_treatment_effectsBP`filenum'.csv", replace

	drop b effect se z pvalue ll ul df crit eform
	...
	****************************************************
	...
	export delimited Expungement Sequence b se pvalue ll ul using "./jobSat_group_probsBP`filenum'.csv", replace
	...
	***************************************************
	...
	export delimited using "./jobSat_ME`filenum'.csv", replace
}
