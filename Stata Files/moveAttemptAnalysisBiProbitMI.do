...
forvalues filenum = 1/`imputations' {
	...
	***********************************
	import delimited "./moveAttemptData`filenum'.csv", clear
	drop if seq==1
		 
	***********************************
	*** Main Bivariate Probit Model ***
	***********************************
	biprobit (move_attempt = i.expunge##i.seq white female age_enroll income_lag house_num house_size unemploy_lag local_unemploy ps_income ps_employed ps_move_attempt married dependent i.year) ///
	(expunge = treatment white female age_enroll income_lag house_num house_size unemploy_lag local_unemploy ps_income ps_employed ps_move_attempt married dependent i.seq i.year), ///
	vce(cluster stid)
	...
	* Time-varying LATEs
	...
	export delimited effect b se pvalue ll ul using "./moveAttempt_treatment_effectsBP`filenum'.csv", replace
	...
	****************************************************
	...
	export delimited Expungement Sequence b se pvalue ll ul using "./moveAttempt_group_probsBP`filenum'.csv", replace
	
	***************************************************
	*** Calculate, Store and Export Model Estimates ***
	***************************************************
	import delimited using "./moveAttemptData`filenum'.csv", clear  //
	drop if seq==1
	...
	export delimited using "./moveAttempt_ME`filenum'.csv", replace
