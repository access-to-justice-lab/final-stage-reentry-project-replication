...
forvalues filenum = 1/`imputations' {
	...
	*** Main LPM Model ***
	**********************
	ivreg2 job_apply (i.expunge i.expunge#seq = i.treatment i.treatment#seq) ///
	white age_enroll female income_lag ps_income unemploy_lag local_unemploy ps_employed married dependent i.seq i.year, ///
		cluster(stid)
	...
	* Time-varying LATEs
	...
	export delimited effect b se pvalue ll ul using "./jobApply_treatment_effectsLPM`filenum'.csv", replace
	...
	****************************************************
	...
	export delimited Expungement Sequence b se pvalue ll ul using "./jobApply_group_probsLPM`filenum'.csv", replace
	...
	***************************************************
	...
	export delimited using "./jobApply_MELPM`filenum'.csv", replace
