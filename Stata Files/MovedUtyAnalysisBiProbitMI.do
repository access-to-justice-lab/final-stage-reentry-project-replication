...
forvalues filenum = 1/`imputations' {
	...
	***********************************
	biprobit (moved_uty = i.expunge##i.seq white age_enroll female income_lag house_sat unemploy_lag ps_employed local_unemploy married dependent i.year) ///
	(expunge = treatment white age_enroll female income_lag house_sat unemploy_lag ps_employed local_unemploy married dependent i.seq i.year), ///
	vce(cluster stid)
	...
	* Time-varying LATEs
	...
	export delimited effect b se pvalue ll ul using "./movedUty_treatment_effectsBP`filenum'.csv", replace
	...
	****************************************************
	...
	export delimited Expungement Sequence b se pvalue ll ul using "./movedUty_group_probsBP`filenum'.csv", replace
	...
	***************************************************
	...
	export delimited using "./movedUty_ME`filenum'.csv", replace
