...
forvalues filenum = 1/`imputations' {
	...
	***********************************
	import delimited "./lifeSatData`filenum'.csv", clear
	
	*********************
	*** Main IV Model ***
	*********************
	ivreg2 life_sat (i.expunge i.expunge#i.seq = i.treatment i.treatment#i.seq) ///
	...
	* Time-varying LATEs
	...
	export delimited effect b se pvalue ll ul using "./lifeSat_treatment_effectsBP`filenum'.csv", replace
	...
	****************************************************
	...
	export delimited Expungement Sequence b se pvalue ll ul using "./lifeSat_group_probsBP`filenum'.csv", replace
	
	***************************************************
	*** Calculate, Store and Export Model Estimates ***
	***************************************************
	import delimited using "./lifeSatData`filenum'.csv", clear  //
	...
	export delimited using "./lifeSat_ME`filenum'.csv", replace
