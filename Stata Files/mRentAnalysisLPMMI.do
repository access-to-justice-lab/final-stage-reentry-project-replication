...
forvalues filenum = 1/`imputations' {
	...
	*** Main LPM Model ***
	**********************
	ivreg2 m_rent (i.expunge i.expunge#seq = i.treatment i.treatment#seq) ///
	white female age_enroll income_lag unemploy_lag local_unemploy ps_income ps_employed married dependent i.seq i.year, ///
		cluster(stid)
	...
	* Time-varying LATEs
	...
	export delimited effect b se pvalue ll ul using "./mRent_treatment_effectsLPM`filenum'.csv", replace
	...
	****************************************************
	...
	export delimited Expungement Sequence b se pvalue ll ul using "./mRent_group_probsLPM`filenum'.csv", replace
	...
	***************************************************
	...
	export delimited using "./mRent_MELPM`filenum'.csv", replace
