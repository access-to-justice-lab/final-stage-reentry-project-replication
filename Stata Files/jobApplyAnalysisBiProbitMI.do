...
forvalues filenum = 1/`imputations' {
	...
	***********************************
	import delimited using "./jobApplyData`filenum'.csv", clear  // 
	drop if seq == 1

	***************************
	*** Main Biprobit Model ***
	***************************
	biprobit (job_apply = i.expunge##i.seq white age_enroll female income_lag ps_income unemploy_lag local_unemploy ps_employed married dependent i.year) ///
	(expunge = treatment white age_enroll female income_lag ps_income unemploy_lag local_unemploy ps_employed married dependent i.seq i.year), ///
	vce(cluster stid)
	...
	* Time-varying LATEs
	...
	export delimited effect b se pvalue ll ul using "./jobApply_treatment_effectsBP`filenum'.csv", replace
	...
	****************************************************
	...
	export delimited Expungement Sequence b se pvalue ll ul using "./jobApply_group_probsBP`filenum'.csv", replace
	...
	***************************************************
	...
	biprobit (full_time = i.expunge##i.seq white age_enroll female income_lag ps_income local_unemploy ps_employed married dependent i.year) ///
	(expunge = treatment white age_enroll female income_lag ps_income local_unemploy ps_employed married dependent i.seq i.year), ///
		vce(cluster stid)
	...
	export delimited using "./jobApply_ME`filenum'.csv", replace
