...
forvalues filenum = 1/`imputations' {
	...
	***********************************
	import delimited "./moveDeterData`filenum'.csv", clear
	drop if seq==1
		 
	***********************************
	*** Main Bivariate Probit Model ***
	***********************************
	biprobit (move_deter = i.expunge##i.seq white female age_enroll income_lag house_num house_size unemploy_lag local_unemploy ps_income ps_employed ps_move_deter married dependent i.year) ///
	(expunge = treatment white female age_enroll income_lag house_num house_size unemploy_lag local_unemploy ps_income ps_employed ps_move_deter married dependent i.seq i.year), ///
	vce(cluster stid)
	...
	* Time-varying LATEs
	...
	export delimited effect b se pvalue ll ul using "./moveDeter_treatment_effectsBP`filenum'.csv", replace
	...
	****************************************************
	...
	export delimited Expungement Sequence b se pvalue ll ul using "./moveDeter_group_probsBP`filenum'.csv", replace
	
	***************************************************
	*** Calculate, Store and Export Model Estimates ***
	***************************************************
	import delimited using "./moveDeterData`filenum'.csv", clear  //
	drop if seq==1
	...
	export delimited using "./moveDeter_ME`filenum'.csv", replace
