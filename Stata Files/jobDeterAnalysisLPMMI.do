...
forvalues filenum = 1/`imputations' {
	...
	*** Main LPM Model ***
	**********************
	ivreg2 job_deter (i.expunge i.expunge#seq = i.treatment i.treatment#seq) ///
	white female age_enroll income_lag full_time_lag unemploy_lag local_unemploy ps_income ps_employed ps_job_deter married dependent i.seq i.year, ///
		cluster(stid)
	...
	* Time-varying LATEs
	...
	export delimited effect b se pvalue ll ul using "./jobDeter_treatment_effectsLPM`filenum'.csv", replace
	...
	****************************************************
	...
	export delimited Expungement Sequence b se pvalue ll ul using "./jobDeter_group_probsLPM`filenum'.csv", replace
	...
	***************************************************
	...
	export delimited using "./jobDeter_MELPM`filenum'.csv", replace
