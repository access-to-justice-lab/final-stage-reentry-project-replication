...
forvalues filenum = 1/`imputations' {
	...
	***********************************
	import delimited "./full_timeData`filenum'.csv", clear
	drop if seq==1
		 
	***********************************
	*** Main Bivariate Probit Model ***
	***********************************
	biprobit (full_time = i.expunge##i.seq white age_enroll female income_lag ps_income full_time_lag local_unemploy ps_employed married dependent i.year) ///
	(expunge = treatment white age_enroll female income_lag ps_income full_time_lag local_unemploy ps_employed married dependent i.seq i.year), ///
		vce(cluster stid)
	...
	* Time-varying LATEs
	...
	export delimited effect b se pvalue ll ul using "./full_time_treatment_effectsBP`filenum'.csv", replace
	...
	****************************************************
	...
	export delimited Expungement Sequence b se pvalue ll ul using "./full_time_group_probsBP`filenum'.csv", replace
	
	***************************************************
	*** Calculate, Store and Export Model Estimates ***
	***************************************************
	import delimited using "./full_timeData`filenum'.csv", clear  //
	drop if seq==1
	...
	export delimited using "./full_time_ME`filenum'.csv", replace
