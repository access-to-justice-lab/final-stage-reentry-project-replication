...
forvalues filenum = 1/`imputations' {
	...
	***********************************
	import delimited "./lifeCompData`filenum'.csv", clear
	
	*********************
	*** Main IV Model ***
	*********************
	ivreg2 life_comp (i.expunge i.expunge#i.seq = i.treatment i.treatment#i.seq) ///
		white female age_enroll income income_lag house_sat job_sat unemploy_lag ps_income ps_employed local_unemploy married dependent i.seq i.year, ///
		cluster(stid)
	...
	* Time-varying LATEs
	...
	export delimited effect b se pvalue ll ul using "./lifeComp_treatment_effectsBP`filenum'.csv", replace
	...
	****************************************************
	...
	export delimited Expungement Sequence b se pvalue ll ul using "./lifeComp_group_probsBP`filenum'.csv", replace
	
	***************************************************
	*** Calculate, Store and Export Model Estimates ***
	***************************************************
	import delimited using "./lifeCompData`filenum'.csv", clear  //
	...
	export delimited using "./lifeComp_ME`filenum'.csv", replace
