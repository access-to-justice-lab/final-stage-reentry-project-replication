...
forvalues filenum = 1/`imputations' {
	...
	***********************************
	import delimited "./mRentData`filenum'.csv", clear
	drop if seq==1
		 
	***********************************
	*** Main Bivariate Probit Model ***
	***********************************
	biprobit (m_rent = i.expunge##i.seq white female age_enroll income_lag unemploy_lag local_unemploy ps_employed married dependent i.year) ///
	(expunge = treatment white female age_enroll income_lag unemploy_lag local_unemploy ps_employed married dependent i.seq i.year), ///
	vce(cluster stid)
	...
	* Time-varying LATEs
	...
	export delimited effect b se pvalue ll ul using "./mRent_treatment_effectsBP`filenum'.csv", replace
	...
	****************************************************
	...
	export delimited Expungement Sequence b se pvalue ll ul using "./mRent_group_probsBP`filenum'.csv", replace
	
	***************************************************
	*** Calculate, Store and Export Model Estimates ***
	***************************************************
	import delimited using "./mRentData`filenum'.csv", clear  //
	drop if seq==1
	...
	export delimited using "./mRent_ME`filenum'.csv", replace
